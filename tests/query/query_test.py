...
class TestStripOdataCount:
    """Test _strip_odata_count with $count=true in various positions and encodings."""

    BASE = "https://catalogue.dataspace.copernicus.eu/odata/v1/Products"

    # Unencoded ($count=true)

    def test_unencoded_first(self) -> None:
        url = f"{self.BASE}?$count=true&$filter=x&$top=10"
        assert _strip_odata_count(url) == f"{self.BASE}?$filter=x&$top=10"

    def test_unencoded_middle(self) -> None:
        url = f"{self.BASE}?$filter=x&$count=true&$top=10"
        assert _strip_odata_count(url) == f"{self.BASE}?$filter=x&$top=10"

    def test_unencoded_last(self) -> None:
        url = f"{self.BASE}?$filter=x&$top=10&$count=true"
        assert _strip_odata_count(url) == f"{self.BASE}?$filter=x&$top=10"

    # Encoded (%24count=true)

    def test_encoded_first(self) -> None:
        url = f"{self.BASE}?%24count=true&%24filter=x&%24top=10"
        assert _strip_odata_count(url) == f"{self.BASE}?%24filter=x&%24top=10"

    def test_encoded_middle(self) -> None:
        url = f"{self.BASE}?%24filter=x&%24count=true&%24top=10"
        assert _strip_odata_count(url) == f"{self.BASE}?%24filter=x&%24top=10"

    def test_encoded_last(self) -> None:
        url = f"{self.BASE}?%24filter=x&%24top=10&%24count=true"
        assert _strip_odata_count(url) == f"{self.BASE}?%24filter=x&%24top=10"

    # No $count present

    def test_no_count(self) -> None:
        url = f"{self.BASE}?$filter=x&$top=10"
        assert _strip_odata_count(url) == url
